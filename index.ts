...
const CanvasBuffer = Vue.extend({
    ...
    computed: {
        prettyFPS() {
            return (this.fps.reduce((acc: number, n: number) => acc + n, 0) / this.fps.length).toFixed(3)
        },
        buffer() {
            if (this.scale === 1) return this.value;
    ...
    methods: {
        render(now: DOMHighResTimeStamp) {
            ...
            while (this.fps.length > 600) this.fps.shift();
            this.last = now;
...
new Vue({
    ...
    data() {
        ...
        return {
            cols, rows, scale: 4,
            buffer: new Array(cols * rows * 4).fill(undefined).map((n: number, i: number) => i % 4 === 3 ? 255 : 0)
        }
    }
}).$mount('#app');
